...
const WalletConnect = () => {
  ...
  // Function to connect or disconnect wallet
  const handleWalletClick = async () => {
    if (account) {
      // ✅ Disconnect: Revoke permissions & clear state
      try {
      ...
      // ✅ Connect: Request access to MetaMask
      if (window.ethereum) {
        try {
  ...
  return (
    <button
      ...
      className="w-[182.99px] h-[42.23px] bg-[#ffffff] rounded-[10.75px] justify-center items-center gap-[4.60px] inline-flex grow shrink basis-0 text-center text-black text-[13.44px] font-[500] font-['Inter']"
    >
      {account ? (
        <>
          <div className="w-[31.23px]  h-[31.23px]   bg-[url(/2p.png)] bg-cover bg-no-repeat bg-center   rounded-full" />
